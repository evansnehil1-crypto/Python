...
while serving:
    ...
    amount = int(input(f"Hello {name}! Enter withrawel amount: "))

    if amount <= 0:

        print("Invalid amount. Please enter a posititve number.\n")
        continue
...
for slot in range(1, 7):
    ...
    if total > 0:
        print(f" {value}-unit nothes dispensed: (total)",end="")
...
print(f"Total dispensed : {total_despensed} units")
